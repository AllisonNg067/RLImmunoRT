...
model.load_weights(action_type + '_ddqn_' + reward_type + '_more_exploration.weights.keras')
...
def training_step(batch_size):
    ...
    # print(next_states)
    # print(dones)
    next_Q_values = target(next_states, training=False)
...
def exploration_probability(episode, explore=0):
    return min(1, max(1 - (episode - explore - 3000)/9000, 0.01))

replay_buffer = deque(maxlen=2000)
with open('replay_buffer_' + action_type + '_ddqn_' + reward_type + '_more_exploration.pkl', 'rb') as f:
...
with open('train_reward_' + action_type + '_ddqn_' + reward_type + '_more_exploration.pkl', 'rb') as f:
    rewards = pickle.load(f)

with open('train_final_reward_' + action_type + '_ddqn_' + reward_type + '_more_exploration.pkl', 'rb') as f:
...
import pickle
with open('replay_buffer_' + action_type + '_ddqn_' + reward_type + '_more_exploration.pkl', 'wb') as f:
    pickle.dump(replay_buffer, f)

with open('train_reward_' + action_type + '_ddqn_' + reward_type + '_more_exploration.pkl', 'wb') as f:
    pickle.dump(rewards, f)

with open('train_final_reward_' + action_type + '_ddqn_' + reward_type + '_more_exploration.pkl', 'wb') as f:
...
plt.show()
plt.tight_layout()
plt.savefig('sum of rewards ' + action_type + ' ddqn ' + reward_type + '_more_exploration.png')
plt.figure(figsize=(8, 4))
...
plt.show()
plt.tight_layout()
plt.savefig('final reward ' + action_type + ' ddqn ' + reward_type + '_more_exploration.png')
model.compile(optimizer='adam', loss='mse')
model.save(action_type + '_ddqn_' + reward_type + '_more_exploration.weights.keras')
